...
function termImg(image, options: any = {}) {
	const fallback =
...
async function render(
	buffer,
	{ width: inputWidth, height: inputHeight, preserveAspectRatio }
) {
	...
	for (let y = 0; y < image.bitmap.height - 1; y += 2) {
		for (let x = 0; x < image.bitmap.width; x++) {
			...
			result +=
				a === 0
					? chalk.reset(" ")
					: chalk.bgRgb(r, g, b).rgb(r2, g2, b2)(PIXEL);
		}
...
const terminalImage: any = {};

terminalImage.buffer = async (
	buffer,
...
terminalImage.gifBuffer = (buffer, options: any = {}) => {
	options = {
	...
	const animation = renderGif(
		buffer,
		async (frameData) => {
			options.renderFrame(
				await terminalImage.buffer(Buffer.from(String(frameData)), options)
			);
		},
		options
	);
